...
public class IPLAnalyserTest {
	...
	@Test
	public void givenCsvDataShouldReturnCricketerWithBestBattingandBowlingAvg() {
		try {
			iplAnalyser.loadPlayerRuns(PLAYER_RUNS_DATA);
			iplAnalyser.loadBowler(BOWLERS_DATA);
			String playerName = iplAnalyser.cricketerWithBestBattingAndBowlingAvg();
			Assert.assertEquals("Andre Russell", playerName);
		} catch (IPLAnalyserException e) {
			e.printStackTrace();
		}
	...
	@Test
	public void givenCsvDataShouldReturnCricketerWithMax100AndBestAvg() {
		try {
			iplAnalyser.loadPlayerRuns(PLAYER_RUNS_DATA);
			String playerName = iplAnalyser.playerWithMax100AndBestAvg();
			Assert.assertEquals("David Warner", playerName);
		} catch (IPLAnalyserException e) {
			e.printStackTrace();
		}
	...
	@Test
	public void givenCsvDataShouldReturnCricketerWithZero100_50AndBestAvg() {
		try {
			iplAnalyser.loadPlayerRuns(PLAYER_RUNS_DATA);
			String playerName = iplAnalyser.playerWithZero100_50AndBestAvg();
